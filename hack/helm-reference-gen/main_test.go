...
func TestAllValues(t *testing.T) {
	inputBytes, err := ioutil.ReadFile(filepath.Join("fixtures", "case1.yaml"))
	require.NoError(t, err)
	expBytes, err := ioutil.ReadFile(filepath.Join("fixtures", "case1.golden"))
	require.NoError(t, err)

	actual, err := Gen(string(inputBytes))
	require.NoError(t, err)
	if actual != string(expBytes) {
		require.NoError(t, ioutil.WriteFile(filepath.Join("fixtures", "case1.actual"), []byte(actual), 0644))
		require.FailNow(t, "output not equal, actual output to case1.actual")
	}
}
